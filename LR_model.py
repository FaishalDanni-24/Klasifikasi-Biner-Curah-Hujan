...
def build_model(random_state_val, use_randomcv):
    ...
    model = LogisticRegression(max_iter=1000, random_state=random_state_val)
...
def show_equation(model, feature_names):
...
def save_model(model, filepath):
    pickle.dump(model, open(f"{filepath}/LR_model.pkl", "wb"))
    print("Best Logistic Regression Model is saved!")
...
def main():
    ...
    print("Using Algorithm: Logistic Regression")
    ...
    # Feature importance
    show_equation(best_rain_classifier_model, features)

    
    # Evaluasi model pada set training
    evaluate_model(best_rain_classifier_model, X_train_sm, y_train_sm, "figures/Model/LR", "Training Set")

    # Evaluasi model pada set test
    evaluate_model(best_rain_classifier_model, X_test, y_test, "figures/Model/LR", "Test Set")
